with category as (select id from quiz_categories where code = 'GENERAL')
insert into quiz_questions (category_id, type, prompt, explanation, difficulty)
select category.id, 'TEXT_MULTIPLE_CHOICE', '다음 중 프로그래밍 언어가 아닌 것은?', 'HTML은 마크업 언어입니다.', 1
from category
where not exists (
  select 1 from quiz_questions where prompt = '다음 중 프로그래밍 언어가 아닌 것은?'
);

with question as (
  select id from quiz_questions where prompt = '다음 중 프로그래밍 언어가 아닌 것은?'
)
...
from question
cross join (values
  ('TypeScript', 'TypeScript', false, 1),
  ('Rust', 'Rust', false, 2),
  ('Python', 'Python', false, 3),
  ('Go', 'Go', false, 4),
  ('HTML', 'HTML', true, 5)
